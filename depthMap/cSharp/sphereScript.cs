...
public class sphereScript : MonoBehaviour {
	...
	public int sphereLimit;
	private string rowOfData;
	public float sphereZdistance;
	...
	void initRays() {
		Ray ray;
		Vector3 rayTarget;
		float maxAngle = Mathf.Atan(randomRangeLimit/sphereZdistance);
		float theta1,theta2;
	...
	void rayCastDataGet() {
		Ray ray;
		RaycastHit hit;
		rowOfData = "1"; // 1 label indicates sphere
		for (int i = 0; i < numSamplesPerSide; i++) {
			for (int j = 0; j < numSamplesPerSide; j++) {
				ray = rays[i,j];
	...
	void Start () {
		initRays ();
		//double test = Accord.Math.Constants.Log2;
	}

	// Update is called once per frame
	void Update () {
		if (counter < sphereLimit) {
			Debug.Log(counter.ToString());
			// randomly move sphere
			Vector3 position = new Vector3(RandomU.Range(-randomRangeLimit, randomRangeLimit), RandomU.Range(-randomRangeLimit, randomRangeLimit), sphereZdistance);
